...
(function() {
	var phrases = {
		...
		'fileError': '上传失败',
		'validationErrors': {
			'duplicate': '相同文件名 <em>{name}</em> 已经添加.',
			'sizeLimitMin': '文件 <em>{name}</em> (<em>{size}</em>) 太小了, 最小不得小于 {fileSizeMin}.',
			'sizeLimitMax': '文件 <em>{name}</em> (<em>{size}</em>) 太大了, 最大不得大于 <em>{fileSizeMax}</em>.',
			'fileListMax': '文件 <em>{name}</em> 无法添加, 一次性最多上传 <em>{fileListMax} 文件</em>.',
