...
<table class="admin_table" width="100%" border="0" cellpadding="0" cellspacing="1" style="margin-top:0px " id="table1">
    <tr>
	<td colspan="9" class="td_title">新闻管理</td>
    </tr>
    <tr>
<td width="5%" style="text-align:center">审核</td>
	...
	<td width="5%"  style="text-align:center">首页显示</td>
        <td width="5%"  style="text-align:center">分会显示</td>
        <td width="7%"  style="text-align:center">删除</td>
    </tr>

    <?php if(count($news) == 0): ?>
    <tr>
        <td colspan="9" style="background-color:#fff;padding:10px; text-align: left; color: #999">没有任何新闻信息。</td>
    </tr>
    <?php else: ?>
...
<span style="color:#999"><?php if($from == 'aa'): ?> [ <?=$n['aa_name']?>]&nbsp;<?php endif; ?><?php if($from == 'club'): ?> [ <?=$n['club_name']?>]&nbsp;<?php endif; ?></span>
            <a href="<?= URL::site('admin_news/form?id='.$n['id']) ?>" title="点击修改" style="<?=$n['title_color']?'color:'.$n['title_color']:null?>"><?= $n['title'] ?></a>
	...
	<td style="text-align: center"><input type="checkbox" value="1" onclick="dig2hp(<?= $n['id'] ?>,1);" <?= $n['is_home'] ? 'checked':'' ?> /></td>
        <td style="text-align: center"><input type="checkbox" value="1" onclick="show_branch(<?= $n['id'] ?>);" <?= $n['is_show_branch'] ? 'checked':'' ?> /></td>
...
<script type="text/javascript">
    ...
    function show_branch(id){
        new Request({
            url: '<?= URL::site('admin_news/showbranch').URL::query() ?>',
            type: 'post',
            data: 'cid='+id
        }).send();
    }
