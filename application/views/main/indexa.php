...
<div class="row">
    ...
    <?php
    if ($news_special) {
        echo View::factory('main/sinaweibo', array(
            'weibolist' => $weibolist,
        ...
        echo View::factory('main/home_news', array(
            'top_news' => $top_news,
            'news_list' => $news_list,
        ));
    }
    ?>
    <div id="qiushi">
        <p class="sidebar_title" style="border-width:1px 0">求是群芳</p>
        ...
        <div style="padding:10px">
            ...
            <div id="zhutihuodong" style="height:240px;overflow: hidden">
                <ul>
                    <?php foreach ($static as $e): ?>
                        <li><?php if (empty($e['redirect'])): ?><a href="<?= URL::site('event/static?id=' . $e['id']) ?>" target="_blank"><?php else: ?><a href="<?= $e['redirect'] ?>" target="_blank"><?php endif; ?><img src="<?= $e['img_path'] ?>" width="220" height="70" style="margin-bottom:10px"/></a></li>
                    <?php endforeach; ?>
                </ul>
            </div>
...
<div class="big_tab_title" id="photo_tabs">
    <ul>
        <li tag="pic"><a href="javascript:void(0)" id="photo1"  onmouseover="sHome.setMorePhoto('morephoto1')" class="cur">求是追忆</a></li>
...
$old_pic_html = '';
if ($old_pic) {
    $old_pic_html = '<table cellspacing="0" cellpadding="0" width="322"  border="0"><tr align="center">';
    foreach ($old_pic as $p) {
        $old_pic_html.='<td width="130" height="110"><p class="a_pic"><a href="/album/picView?id=' . $p['id'] . ' target="_blank"><img src="http://zuaa.zju.edu.cn/' . $p['img_path'] . '" width="105" height="79" /></a></p></td>';
    }
    $old_pic_html.='</tr></table>';
}
...
if ($event_pic) {
    ...
    foreach ($event_pic as $p) {
        $event_pic_html.='<td width="130" height="110"><p class="a_pic"><a href="/album/picView?id=' . $p['id'] . ' target="_blank"><img src="http://zuaa.zju.edu.cn/' . $p['img_path'] . '" width="105" height="79" /></a></p></td>';
    }
    ...
    <?php else: ?>
        <div id="demo" style="overflow: hidden; width: 895px; height: 110px">
            <table cellpadding="0" align="left" border="0" cellspacing="0">
                <tr>
                    <td id="demo1" valign="top" style="display:none"></td>
...
<script type="text/javascript">
    ...
    readyScript.body = function() {
        ...
        $(window).scroll(function() {
            ...
            if (scrollTop > 220 && $demo1.html() === '') {
                $demo1.html('<?= $old_pic_html ?>').fadeIn(500, function() {
                    setTimeout(function() {
        ...
        $('#photo_tabs li[tag]').mouseover(function() {
            var curLink = $(this).find('a').addClass('cur');
            var $demo4 = (typeof $demo4) === 'Object' ? $demo4 : $('#demo4');
            ...
            //延迟加载活动照片
            if (curLink.attr('id') === 'photo2' && $demo4.html() === '') {
                $demo4.html('<?= $event_pic_html ?>').fadeIn(500, function() {
                    setTimeout(function() {
...
<?php if (!$news_special): ?>
            //$("#marquee3 .donate_tks").RollTitle({line: 8, speed: 400, timespan: 1500});
<?php endif; ?>
...
<?php if ($focus_title): ?>sHome.focusNews({titles: '<?= preg_replace('/\s*/', '', $focus_title) ?>', imgs: '<?= $focus_imgs ?>', urls: '<?= $focus_url ?>'});<?php endif; ?>
        setTimeout(function() {
            ...
            $("#zhutihuodong ul").RollTitle({line: 1, speed: 400, timespan: 1500});
            $("#marquee2 .donate_tks").RollTitle({line: 8, speed: 400, timespan: 1500});
        }, 1500);
    };
</script>

<? if (date('m-d') == '09-09' OR date('m-d') == "09-10"): ?>
    <DIV id="img1" style="z-index: 100; left: 2px; width: 180px; position:  absolute; top:  43px; height:180px;visibility: visible;"><a href="http://huati.weibo.com/425014?from=510&order=time" target="_blank"><img src="/static/images/jsj2.png" width="180" height="180" border="0"></a></DIV>
    <script type="text/javascript" src="/static/js/jsj.js"></script>
<?php endif; ?>

<div style="position: fixed;width:50px;height:120px;bottom: 225px;right: 70px;border: 0px solid #f60">
    <ul id="siteapptool">
        <li>
            <a href="#" class="sinaweibo"  ></a>
            <div class="app" >
                <div class="box" ><em>&#9670;</em><span>&#9670;</span>
                    <img src="/static/images/zuaa_weibo.jpg" width="150px" height="150px"/>
                    <div>浙江大学校友总会<br>微博</div>
                </div>
            </div>
        </li>
        <li>
            <a href="#" class="tentweixin"  ></a>
            <div class="app" style=" display:none">
                <div class="box" ><em>&#9670;</em><span>&#9670;</span>
                    <img src="/static/images/zuaa_weibo.jpg" width="150px" height="150px"/>
                    <div>浙江大学校友总会<br>微信</div>
                </div>
            </div>
        </li>
    </ul>
...
<script type="text/javascript">
    (function() {
        $siteapptool = $("#siteapptool");
        $siteapptool.find('li a.sinaweibo').mouseover(function(event) {
            event.stopPropagation();
            if ($(this).is(":animated") == false) {
                $(this).css({backgroundPosition: "-65px 0"}).animate({
                    backgroundPositionY: '-62px'
                }, 250).parent().find('.app').show();
            }
        }).mouseleave(function() {
            $(this).css({backgroundPosition: "0 -62px"}).animate({
                backgroundPositionY: '0'
            }, 250).parent().find('.app').hide();
        });

        $siteapptool.find('li a.tentweixin').mouseover(function(event) {
            event.stopPropagation();
            if ($(this).is(":animated") == false) {
                $(this).css({backgroundPosition: "-65px -206px"}).animate({
                    backgroundPositionY: '-144px'
                }, 250).parent().find('.app').show();
            }
        }).mouseleave(function() {
            $(this).css({backgroundPosition: "0 -144px"}).animate({
                backgroundPositionY: '-206px'
            }, 250).parent().find('.app').hide();
        });
    })();
