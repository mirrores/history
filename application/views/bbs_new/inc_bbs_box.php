...
<div class="forum_box_list">
	...
	<?php $display=$display+1;?>
		    <div class="forum_box">
			<div class="left"><a href="<?= URL::site('bbs/list?f=' . $from) . '&b=' . $b['id'] ?>"><img src="/static/images/forum<?php if ($b['today_count'] OR date('Y-n-d', strtotime($aa_bbs_unit[$b['id']]['comment_at'])) == date('Y-n-d')): ?>_new<?php endif; ?>.gif"></a></div>
		    	<div class="right">
		    	    <a href="<?= URL::site('bbs/list?f=' . $from) . '&b=' . $b['id'] ?>" class="name"><?= $b['name'] ?></a><span class="today_count"><?= $b['today_count'] ? '(今日' . $b['today_count'] . ')' : '' ?></span><br>
