...
<div style="margin:15px 0;height: 30px">
    ...
    <div style="width:100px;float: right;text-align: right;padding-right: 20px">
    ...
    <?php else:?>
    <input type="button"   type='button' class="button_disabled"  disabled="disabled"   value="临时关闭发帖">
    <?php endif;?>
    </div>
            ...
            <?php foreach ($fixed_units as $i => $u): ?>
                <tr <?= $i % 2 == 1 ? 'style="background-color:#F9F9F9"' : null ?>>
                    <td style="font-size: 1.1em; color:#999; text-align: left;"> &nbsp;
                        ...
                        <a href="<?= URL::site('bbs/view' . $u['type'] . '?id=' . $u['id']) ?>" class="title" <? if ($u['is_fixed'] OR $u['is_good']): ?>style="font-weight:bold;color:<?= $u['title_color'] ? $u['title_color'] : '#f30'; ?>"<?php else: ?> <?php if (!empty($u['title_color'])): ?>style="font-weight:bold;color:<?= $u['title_color'] ?>"<?php endif; ?><?php endif; ?>   ><?= Text::limit_chars($u['title'], 35, '...') ?></a>
...
<?php if(!$_SETTING['prohibit_posting']) :?>
<input type="button" type='button' class="unit_post" onclick="window.location.href = '<?= URL::site('bbs/unitForm?aa_id=' . $aa_id . '&club_id=' . @$club_id . '&b=' . $bbs_id) ?>'" value="发布新帖">
    <?php else: ?>
<input type="button"  type='button' class="button_disabled"  disabled="disabled"   value="临时关闭发帖">
    <?php endif; ?>
    </div>
        ...
        <form method="post" action="<?= URL::site('bbs/unitPost') ?>" id="bbs_unit_form" style="margin:6px 15px">
            ...
            <table >
                ...
                <tr>
                    <td ></td>
                    <td>
<input type="button" id="submit_button"   value="发布帖子"   onclick="unitSave()" class="button_blue" />
                    </td>
                </tr>
            </table>
        </form>
