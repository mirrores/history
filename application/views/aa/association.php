...
<div class="blue_tab" style="margin: 15px 20px">
    <ul>
        ...
        <li><a href="<?= URL::site('aa/association') ?>" id="one5" class="cur">学生协会</a></li>
    </ul>
</div>
...
<div id="con_one_3" class="tab_content" style="height: 450px">
    <?php if (!$association): ?>
        <p class="nodata">很抱歉，学生协会还在整理当中。</p>
    <? else: ?>
        <ul class="aa_institution">
            <?php foreach ($association as $i): ?>
            <li><a href="<?= URL::site('aa_home?id=' . $i['id']); ?>"  onmouseover="wsug(event, '<?= $i['name'] ?><br>联系人：<?= $i['contacts'] ? $i['contacts'] : '-'; ?><br>电话：<?= $i['tel'] ? $i['tel'] : '-'; ?><br>邮件：<?= $i['email'] ? $i['email'] : '-'; ?><br>地址：<?= $i['address'] ? $i['address'] : '-'; ?>')" onmouseout="wsug(event, 0)" target="_blank"><?= $i['name'] ?></a></li>
            <?php endforeach; ?>
        </ul>
    <?php endif ?>
</div>
