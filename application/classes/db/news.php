...
class Db_News {
    ...
    public static function getMobileRecommend($condition) {
        ...
        //指定校友会id
        if (isset($condition['aa_id']) AND $condition['aa_id']>0) {
            $recommend = $recommend->where('c.aa_id', '=', $condition['aa_id']);
        }
        //校友总会
    ...
    public static function getFocus($limit = 3) {
        $news = DB::select(DB::expr('id,title,img_path'))
                ->from(DB::expr('news'))
                ->where('is_draft', '=', 0)
                ->where('is_focus', '=', 1)
                ->order_by('id', 'desc')
    ...
    public static function getMobileList($condition) {
        ...
        //指定校友会总会
        if ($condition['aa_id']>0) {
            $query = $query->where('c.aa_id', '=', $condition['aa_id']);
        }
        //总会新闻
        elseif ($condition['cat'] == 'main') {
