...
class Db_Event {
    ...
    //允许自建队伍或分类要求
    public static function create_procession($event) {
        if (isset($event['club_id']) AND isset($event['sign_limit']) AND $event['club_id'] == 37 AND $event['sign_limit'] >= 100) {
            return true;
        } else {
            return false;
        }
    ...
    public static function getCategoryById($id) {
        $category = DB::select(DB::expr('c.*,(SELECT SUM(s.num) FROM event_sign s WHERE s.category_id = c.id) AS total_sign'))
                ->from(array('event_sign_categorys', 'c'))
                ->where('c.id', '=', $id)
                ->execute()
                ->as_array();
        return $category ? $category[0] : array();
    }
    ...
    public static function getEventSigner($id, $limit = null, $category_id = null) {
        ...
        $signer = Doctrine_Query::create()
                ->select('s.*, u.id, u.realname, u.account,u.sex,c.name AS group_name')
                ->addSelect('(SELECT o.id FROM Ol o WHERE o.uid=s.user_id AND o.time>' . $alive . ' ) AS online')
                ->from('EventSign s')
                ->leftJoin('s.User u')
                ->leftJoin('s.EventSignCategorys c')
                ->where('event_id = ?', $id);
        ...
        $signer = $signer->orderBy('s.sign_at ASC')
                ->fetchArray();
        return $signer;
    }

    //获取活动报名信息（详细：包含工作电话信息）
    public static function getEventSignerMore($id, $limit = null, $category_id = null) {
        $signer = Doctrine_Query::create()
                ->select('s.*,u.id,u.realname,u.account,u.speciality,u.start_year,u.sex,u.city,c.mobile')
                ->from('EventSign s')
                ->leftJoin('s.User u')
        ...
        $signer = $signer->orderBy('s.is_captain DESC,s.sign_at ASC')
                ->fetchArray();
        return $signer;
    }
    ...
    public static function delete($id) {
        ...
        //分类
        DB::delete('event_sign_categorys')->where('event_id', '=', $id)->execute();
    ...
    //是否为某小队队长(包含总管理员或分会或俱乐部管理员)
    public static function is_category_manager($category_id, $user_id, $category = null) {
        ...
        if ($category['captain_id'] == $user_id) {
            return true;
        } else {
            $permission = self::getPermission($category['event_id'], $user_id);
...
//返回当前用户对某一活动的控制权限
    ...
    public static function getEventViewData($condition) {
        ...
        if (isset($condition['user_sign_status']) AND $condition['user_sign_status']) {
            ...
            $data['user_sign_status']['info'] = Doctrine_Query::create()
                    ->from('EventSign')
                    ->where('event_id=?', $event_id)
