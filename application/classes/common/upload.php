    ...
    public static function pic($file_data = null, $user_id = null, $add_watermark = false, $resize = array()) {
        ...
        if ($file_data) {
            ...
            if (!$error) {
                ...
                //另外生成一张地址受保护的图片
                if(isset($config['protected'])) {
                    $protected_str=rand(100, 10000);
                    if ($w > $config['protected']['width']) {
                        Image::factory($upload_file_path)
                                ->resize($config['protected']['width'], $config['protected']['height'])
                                ->save($path . $file_name .$protected_str.'_protected' . $file_extend);
                    } else {
                        copy($upload_file_path, $path . $file_name .$protected_str. '_protected' . $file_extend);
                    }
                    $images['protected'] = $path . $file_name .$protected_str. '_protected' . $file_extend;
                }
