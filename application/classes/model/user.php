...
class Model_User {
    ...
    public static function login($data) {
        ...
        //使用md5 password
        elseif (isset($data['account']) AND isset($data['md5password']) AND $data['account']) {
            $u = $query->where('account', '=', $data['account'])->limit(1)->execute()->as_array();
        }
        //使用令牌登录
        ...
        if ($u) {
            ...
            if (isset($data['account'])) {
                ...
                //网站注册和通过网站登录
                if (isset($data['password']) AND $user['password'] == md5($data['password'])) {
                    $login_success = True;
                }
                //匹配md5密码(32位小写字符串)
                elseif (isset($data['md5password']) AND $user['password'] === $data['md5password']) {
                    $login_success = True;                  
                }
...
//加入的校友会数组
    ...
    public static function register($post) {
        ...
        $files = Model_Alumni::getOneAlumni($post);
