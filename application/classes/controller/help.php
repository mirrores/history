    ...
    function action_file() {
        ...
        if ($_POST) {
            ...
            if ($name) {
                ...
                if ($begin_year) {
                    $files->addWhere('a.begin_year= ?', (int)$begin_year);
                }

                if ($graduation_year) {
                    $files->addWhere('a.graduation_year= ?', (int)$graduation_year);
                }

                if ($speciality) {
                    $files->addWhere('a.speciality LIKE ?', '%' . trim($speciality) . '%');
                }

                if ($institute) {
                    $files->addWhere('a.institute LIKE ?', '%' . trim($institute) . '%');
                }

                $view['files']=$files->limit(1000)->fetchArray();
