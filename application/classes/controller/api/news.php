...
class Controller_Api_News extends Layout_Mobile {
    ...
    function action_index() {
        ...
        //指定校友会id
        if ((int) $condition['aa_id'] > 0) {
            $aa=  Db_Aa::getInfoById($condition['aa_id']);
            if($aa){
                $data['description'] = $aa['name'].'新闻';
            }
            else{
                $data['description'] = '未知校友会新闻';
            }
        }
        //校友总会
        elseif ($condition['cat'] == 'main') {
        ...
        //其次默认
        elseif ($condition['cat'] == 'main' AND $condition['page']==1) {
    ...
    function action_view() {
        ...
        if ($news) {
            $original['content']= Common_Global::relativelyToAbs($news['content']);
            $data['id'] = $news['id'];
            ...
            $data['content'] = $this->_format=='json'?$original['content']:$data['content'];
