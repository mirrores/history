...
class Controller_Upload extends Layout_Main {
    ...
    function action_uploadAttachedfile() {
        ...
        if ($_FILES) {
            ...
            //向浏览器返回数据json数据
            $file = str_replace('../', '', $upload_file_path);  //为方便理解，替换掉所有类似../和./等相对路径标识original
    ...
    function action_frame() {
        ...
        if ($_POST) {
            ...
            if ($_FILES['file']['size'] > 0) {
                ...
                if (!$valid->check()) {
                    $view['error'] .= $valid->outputMsg($valid->errors('validate'));
                } else {
                    // 处理附件
    ...
    function action_frameimg() {
        ...
        if ($_FILES AND $_FILES['file']['size'] > 0) {
            ...
            //上传成功
                ...
                //返回受保护的图片路径
                if (isset($upload_images['images']['protected'])) {
                    $view['file_path_protected'] = $upload_images['images']['protected'];
                }
            }
        } else {
        }
